...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        print("[==>] Received incoming connection from %s:%d".format(addr[0], addr[1]))

        # Start thread to talk to rmote host
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        ...
        if len(remote_buffer):
            print("[<==] Send %d bytes to localhost.".format(len(remote_host)))
